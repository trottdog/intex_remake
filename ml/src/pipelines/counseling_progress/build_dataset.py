"""Build the counseling progress dataset."""
...
def build_dataset(*, save_output: bool = True) -> pd.DataFrame:
    """Build the counseling-progress modeling dataset."""

    config = load_pipeline_config(CONFIG_PATH)
    return build_resident_phase_d_dataset(
        dataset_name=str(config["dataset_name"]),
        target_col=str(config["target"]),
        complete_flag="future_window_complete_90d",
        eligibility_fn=lambda frame: frame["process_recent_90d_count"] >= 2,
...
def main() -> None:
    ...
    print(f"Built counseling progress dataset with {len(dataset)} rows")
