"""Build the donor retention dataset."""
...
def build_dataset(*, save_output: bool = True) -> pd.DataFrame:
    """Build the donor-retention modeling dataset."""

    config = load_pipeline_config(CONFIG_PATH)
    dataset = load_processed_table("supporter_features")
    dataset = dataset.loc[dataset["has_any_donation"]].copy()
...
def main() -> None:
    ...
    print(f"Built donor retention dataset with {len(dataset)} rows")
