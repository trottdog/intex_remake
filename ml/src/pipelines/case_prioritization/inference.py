"""Inference helpers for case prioritization."""
...
from ml.src.pipelines.case_prioritization.build_dataset import build_dataset
from ml.src.pipelines.common import load_pipeline_config

CONFIG_PATH = Path(__file__).with_name("config.yaml")


def score_case_prioritization(
	dataset: pd.DataFrame,
	*,
	include_features: bool = True,
) -> pd.DataFrame:
	"""Score case-prioritization rows and return prediction fields."""

	scored = predict_dataframe(dataset, pipeline_name="case_prioritization")
	if include_features:
		return scored
...
def score_latest_snapshot(
	*,
	...
	"""Build and score the latest case-prioritization snapshot rows."""
	...
	return score_case_prioritization(latest, include_features=include_features)
