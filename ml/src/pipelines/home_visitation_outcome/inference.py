"""Inference helpers for home visitation outcome."""
...
from ml.src.pipelines.home_visitation_outcome.build_dataset import build_dataset

CONFIG_PATH = Path(__file__).with_name("config.yaml")


def score_home_visitation_outcome(
	dataset: pd.DataFrame,
	*,
	include_features: bool = True,
) -> pd.DataFrame:
	"""Score home-visitation-outcome rows and return prediction fields."""

	scored = predict_dataframe(dataset, pipeline_name="home_visitation_outcome")
	if include_features:
		return scored
...
def score_latest_snapshot(
	*,
	...
	"""Build and score the latest home-visitation-outcome snapshot rows."""
	...
	return score_home_visitation_outcome(latest, include_features=include_features)
