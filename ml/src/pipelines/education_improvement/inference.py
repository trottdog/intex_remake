"""Inference helpers for education improvement."""
...
from ml.src.pipelines.education_improvement.build_dataset import build_dataset

CONFIG_PATH = Path(__file__).with_name("config.yaml")


def score_education_improvement(
	dataset: pd.DataFrame,
	*,
	include_features: bool = True,
) -> pd.DataFrame:
	"""Score education-improvement rows and return prediction fields."""

	scored = predict_dataframe(dataset, pipeline_name="education_improvement")
	if include_features:
		return scored
...
def score_latest_snapshot(
	*,
	...
	"""Build and score the latest education-improvement snapshot rows."""
	...
	return score_education_improvement(latest, include_features=include_features)
